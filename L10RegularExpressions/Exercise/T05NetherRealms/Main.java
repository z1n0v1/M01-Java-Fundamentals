/*
        https://judge.softuni.org/Contests/Compete/Index/1673#4

        05. Nether Realms

Mighty battle is coming. In the stormy nether realms, demons are fighting against each other for supremacy in a duel from
...
(e.g. +10 is 10 and -10 is -10). However, there are some symbols ('*' and '/') that can further alter the base damage
...
// and then you need to multiply it by 2 (e.g. 10 * 2 = 20) and then divide it by 2 (e.g. 20 / 2 = 10)).
   ...
   and in the order they appear in the name.
        ...
        •	Number in a demon's name is considered everything that is a valid integer or floating point number
        (with dot '.' used as separator). For example, all these are valid numbers: '4', '+4', '-4', '3.5', '+3.5', '-3.5'
 */

package L10RegularExpressions.Exercise.T05NetherRealms;
...
public class Main {
    public static void main(String[] args) {
        ...
        demons.stream().sorted().forEach(demon -> {
            System.out.printf("%s - %d health, %.2f damage%n", demon, getHealth(demon), getDamage(demon));
        });
    }
    ...
    private static double getDamage(String demon) {
        ...
        Matcher numbers = Pattern.compile("[+-]?[0-9]+\\.?[0-9]*").matcher(demon); // mine
        Matcher operators = Pattern.compile("([\\*\\/])").matcher(demon);

        while (numbers.find()) damage += Double.parseDouble(numbers.group());
        while (operators.find()) {
            char operator = operators.group().charAt(0);
            if (operator == '*') damage *= 2;
            else damage /= 2;
        }
    ...
    private static int getHealth(String demon) {
        Matcher characters = Pattern.compile("([^\\+\\-\\*\\/0-9\\.])").matcher(demon);
        int health = 0;
        while (characters.find()) {
            health += characters.group().charAt(0);
        }
        return health;
    }
}
