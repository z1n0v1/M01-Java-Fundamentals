/*
    https://judge.softuni.org/Contests/Compete/Index/1670#7
...
Depending on whether the letter was in front of the number or after it you would perform
...
He decided to complicate it a bit by doing the same but with multiple strings keeping track of only
the total sum of all results. Once he started to solve this with more strings and bigger numbers it became
quite hard to do it only in his mind. So he kindly asks you to write a program that calculates the sum of all numbers
...
So we add 2 to the resulted number (12+2=14). Similarly for the second string s is lowercase and its position is 19
so we multiply it with the number (17*19 = 323).
...
package L09TextProcessing.Exercise.T08LettersChangeNumbers;
...
//        System.out.println(calculate("s17G"));
//        System.out.println(calculate("A12b"));
    ...
    private static double calculate(String string) {
        ...
        char letterBefore = numberString.charAt(0);
        numberString.deleteCharAt(0);
        char letterAfter = numberString.charAt(numberString.length() -1);
        numberString.deleteCharAt(numberString.length() - 1);

        double number = Integer.parseInt(numberString.toString());
        char position = 0;

        if (Character.isUpperCase(letterBefore)) {
            double divisor = letterBefore - 'A' + 1;
            number /= divisor;
        } else {
            double multiplier = letterBefore - 'a' + 1;
            number *= multiplier;
        }
        if (Character.isUpperCase(letterAfter)) {
//            number -= position;
            double subtract = letterAfter - 'A' + 1;
            number -= subtract;
        } else {
//            number += position;
            double adder = letterAfter - 'a' + 1;
            number+=adder;
        }
